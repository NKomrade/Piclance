  ...
  return (
    <div className="flex space-x-6 p-6 overflow-x-auto">
      {cards.map((card, index) => (
        <div
          key={index}
          className="relative w-80 h-[500px] rounded-xl overflow-hidden shadow-lg"
          style={{ backgroundImage: `url(${card.image})`, backgroundSize: 'cover', backgroundPosition: 'center' }}
        >
          <div className="absolute inset-0 bg-black bg-opacity-50 flex flex-col items-start p-4">
            <h3 className="text-sm text-white pl-2 mb-2">{card.heading}</h3>
            <p className="text-[32px] font-semibold pl-2 text-gray-200">{card.text}</p>
